...
# CONF-PERF-001: current-byte validation precedes inert historical comparison.
PERFORMANCE_AUTHORITY = "2c2e855548fe9dfcc79838d6036472ba86f30cbf972dfebba5fc8d45bed9d699"
...
def performance_reconstruct(path, before, row):
    ...
    for name in spec["regions"]:
        start, end, old = performance_region(before, name)
    ...
    if row["append"]:
        ...
        existing = {n.name for n in ast.parse(before).body if isinstance(n, definitions)}
    ...
    performance_require(set(performance_ids(before)) <= set(performance_ids(current)), "old test lost")
    performance_require(digest(current) == row["afterSha256"], "after hash")
    return current


def performance_proof(sources):
    """Pure six-source oracle. Stored source strings are parsed, never executed."""
    performance_require(type(sources) is dict and set(PERFORMANCE_SPECS) <= set(sources), "six actual sources")
    ...
    performance_require(canonical(proof) == raw and proof["schemaVersion"] == "planeon.conformance-performance-delta/v1"
    ...
    for path in sorted(python_paths):
        ...
        performance_require(type(sources[path]) is bytes and performance_reconstruct(path, before, proof["sources"][path]) == sources[path], "actual delta")
    ...
    added = sorted(set(performance_ids(sources[PERFORMANCE_TEST])) - set(performance_ids(historical[PERFORMANCE_TEST])))
