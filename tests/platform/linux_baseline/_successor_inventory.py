...
# CONF-PERF-004: current-byte validation precedes inert historical comparison.
PERFORMANCE_AUTHORITY = "b3a81dd4e771580d13ba31dcac8405e91336a2229f7845a637de29f927e6672e"
...
def performance_proof(sources):
    ...
    performance_require(canonical(proof) == raw and proof["schemaVersion"] == "planeon.conformance-performance-delta/v4"
    ...
    performance_require(set(PERFORMANCE_REQUIRED_NEW_IDS) <= set(added), "required regression identities")
    begin, end, _ = performance_region(sources[PERFORMANCE_TEST],
        "PerformanceArithmeticTests.test_fixed_three_sample_workload")
    performance_require(digest(sources[PERFORMANCE_TEST][begin:end]) == PERFORMANCE_BENCHMARK_SHA256,
                        "exact matched benchmark method")
    forbidden = {"setUpModule", "tearDownModule", "_performance_finish_profile"}
    performance_require(not any(isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and n.name in forbidden
        for n in ast.parse(proof["sources"][PERFORMANCE_TEST]["append"]).body), "no full-module profiling overlay")
